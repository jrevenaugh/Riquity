...
rqtSolver <- function(igrid) {
  ...
  iStep <- iStart
  grid <- matrix(igrid, nrow = 14, ncol = 15, byrow = TRUE)
  nTried <- rep(0, 14)
  ...
  while(iStep <= 13) {
    ...
    if (nJumps[iStep] == 0 | nTried[iStep] == nJumps[iStep]) {            # No candidates
      if (iStep == iStart) return(list(solution = FALSE,                  # There's no solution
                                       jumpList = NA))
  ...
  return(list(solution = TRUE,
              jumpList = jumpList[iStart:13,2:1]))
}
